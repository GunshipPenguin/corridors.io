function cell (posX, posY) {
  ...
  this.possibleCells = function () {
    var possibleCells = []
    if (this.posX !== 0 && grid[this.posY][this.posX - 1].walked === false) {
      possibleCells.push(grid[this.posY][this.posX - 1])
    } // leftX
    if (this.posY !== 0 && grid[this.posY - 1][this.posX].walked === false) {
      possibleCells.push(grid[this.posY - 1][this.posX])
    } // topY
    if (this.posX !== GRIDSIZEX - 1 && grid[this.posY][this.posX + 1].walked === false) {
      possibleCells.push(grid[this.posY][this.posX + 1])
    } // rightX
    if (this.posY !== GRIDSIZEY - 1 && grid[this.posY + 1][this.posX].walked === false) {
      possibleCells.push(grid[this.posY + 1][this.posX])
    } // bottomY
    return possibleCells
  }

  // Marks this cell as walked, chooses an adjancant cell, removes the
  // appropriate wall, and returns that cell, if no cells are avaliable, return
  // false
